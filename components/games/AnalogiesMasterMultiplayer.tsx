...
/**
 * Analogies Master - Multiplayer Edition
 * Competitive word relationship puzzles
 */
...
interface AnalogyQuestion {
    id: string
    content: { analogy: string }
    options: string[]
    correctAnswer: number
    points: number
}

export default function AnalogiesMasterMultiplayer() {
    ...
    const handlePlayerAnswer = (side: PlayerSide, answerIndex: number) => {
        ...
        const currentQuestion = state.questions[state.currentIndex] as AnalogyQuestion
    ...
    if (gameState === GameState.SETUP) {
        return <PlayerSetupLobby onStartGame={handleStartGame} gameName="Analogies Master Battle" />
    }
...
interface AnalogiesMasterPlayerSideProps {
    state: PlayerGameState
    onAnswer: (answerIndex: number) => void
    playerColor: string
}

function AnalogiesMasterPlayerSide({ state, onAnswer, playerColor }: AnalogiesMasterPlayerSideProps) {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-yellow-50 to-orange-50 rounded-xl p-8 shadow-lg">
                <div className="text-2xl font-bold text-center text-gray-900 leading-relaxed">
                    {currentQuestion.content.analogy}
                </div>
            </div>
