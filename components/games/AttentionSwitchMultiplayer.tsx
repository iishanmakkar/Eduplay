...
/**
 * Attention Switch - Multiplayer Edition
 * Competitive attention switching and counting
 */
...
interface AttentionQuestion {
    id: string
    content: { task: string; items: string[] }
    correctAnswer: number
    points: number
}

export default function AttentionSwitchMultiplayer() {
    ...
    const handlePlayerAnswer = (side: PlayerSide, userCount: number) => {
        ...
        const currentQuestion = state.questions[state.currentIndex] as AttentionQuestion
    ...
    if (gameState === GameState.SETUP) {
        return <PlayerSetupLobby onStartGame={handleStartGame} gameName="Attention Switch Battle" />
    }
...
interface AttentionSwitchPlayerSideProps {
    state: PlayerGameState
    onAnswer: (count: number) => void
    playerColor: string
}

function AttentionSwitchPlayerSide({ state, onAnswer, playerColor }: AttentionSwitchPlayerSideProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-cyan-50 to-blue-50 rounded-xl p-6 shadow-lg">
                <div className="text-2xl font-bold text-center mb-4">
                    {currentQuestion.content.task}
                </div>
