...
/**
 * Creative Story - Multiplayer Edition
 * Competitive creative story completion
 */
...
interface StoryQuestion {
    id: string
    content: { prompt: string; context: string }
    options: string[]
    correctAnswer: number
    points: number
}

export default function CreativeStoryMultiplayer() {
    ...
    const handlePlayerAnswer = (side: PlayerSide, answerIndex: number) => {
        ...
        const currentQuestion = state.questions[state.currentIndex] as StoryQuestion
    ...
    if (gameState === GameState.SETUP) {
        return <PlayerSetupLobby onStartGame={handleStartGame} gameName="Creative Story Battle" />
    }
...
interface CreativeStoryPlayerSideProps {
    state: PlayerGameState
    onAnswer: (answerIndex: number) => void
    playerColor: string
}

function CreativeStoryPlayerSide({ state, onAnswer, playerColor }: CreativeStoryPlayerSideProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-pink-50 to-rose-50 rounded-xl p-6 shadow-lg">
                <div className="text-sm text-gray-600 mb-2">{currentQuestion.content.context}</div>
                <div className="text-xl font-bold mb-4">{currentQuestion.content.prompt}</div>
            </div>
            ...
            <div className="space-y-3">
                {currentQuestion.options.map((option, index) => (
                    <button
                        ...
                        className={`w-full p-4 text-left font-bold rounded-xl transition-all touch-target ${showFeedback
