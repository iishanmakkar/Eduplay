...
/**
 * Logic Puzzle - Multiplayer Edition  
 * Competitive brain teaser challenges with clues
 */
...
export default function LogicPuzzleMultiplayer() {
    ...
    const handlePlayerAnswer = (side: PlayerSide, answerIndex: number) => {
        ...
        const currentQuestion = state.questions[state.currentIndex] as LogicPuzzleQuestion
    ...
    if (gameState === GameState.SETUP) {
        return <PlayerSetupLobby onStartGame={handleStartGame} gameName="Logic Puzzle Battle" />
    }
...
interface LogicPuzzlePlayerSideProps {
    state: PlayerGameState
    onAnswer: (answerIndex: number) => void
    playerColor: string
}

function LogicPuzzlePlayerSide({ state, onAnswer, playerColor }: LogicPuzzlePlayerSideProps) {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 shadow-lg">
                <div className="text-sm text-gray-600 mb-4 text-center">
                    Puzzle {state.currentIndex + 1} of {state.questions.length}
                </div>
                ...
                <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4">
                    <div className="font-semibold text-blue-900 mb-2 text-sm">Clues:</div>
            ...
            <div className="space-y-3">
                {currentQuestion.options.map((option, index) => (
                    <button
                        ...
                        className={`w-full p-4 text-lg font-semibold rounded-xl transition-all touch-target ${showFeedback
                                ? index === currentQuestion.correctAnswer
                                    ? 'bg-green-500 text-white scale-105'
                                    : index === selectedAnswer
                                        ? 'bg-red-500 text-white'
                                        : 'bg-gray-200 text-gray-400'
                                : 'bg-white border-4 hover:scale-102 text-gray-900'
                            }`}
                        style={{
