...
/**
 * Shape Constructor - Multiplayer Edition
 * Competitive shape building and recognition
 */
...
interface ShapeQuestion {
    id: string
    content: { instruction: string; pieces: string[] }
    options: string[]
    correctAnswer: number
    points: number
}

export default function ShapeConstructorMultiplayer() {
    ...
    const handlePlayerAnswer = (side: PlayerSide, answerIndex: number) => {
        ...
        const currentQuestion = state.questions[state.currentIndex] as ShapeQuestion
    ...
    if (gameState === GameState.SETUP) {
        return <PlayerSetupLobby onStartGame={handleStartGame} gameName="Shape Constructor Battle" />
    }
...
interface ShapeConstructorPlayerSideProps {
    state: PlayerGameState
    onAnswer: (answerIndex: number) => void
    playerColor: string
}

function ShapeConstructorPlayerSide({ state, onAnswer, playerColor }: ShapeConstructorPlayerSideProps) {
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-red-50 to-pink-50 rounded-xl p-6 shadow-lg">
                ...
                <div className="flex justify-center gap-4 mb-6 text-6xl">
                    {currentQuestion.content.pieces.map((piece, i) => (
                        <div key={i}>{piece}</div>
                    ))}
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
                {currentQuestion.options.map((option, index) => (
                    <button
                        ...
                        className={`text-6xl p-6 rounded-xl transition-all touch-target ${showFeedback
