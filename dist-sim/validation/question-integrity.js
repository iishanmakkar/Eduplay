...
exports.validateQuestionIntegrity = validateQuestionIntegrity;
exports.validateBatch = validateBatch;
exports.normaliseAnswer = normaliseAnswer;
exports.answersMatch = answersMatch;
 ...
 */
function evaluateFormula(formula, seedParams) {
    try {
...
function validateQuestionIntegrity(q) {
    ...
    if (q.isGenerative && q.seedParams && q.answerFormula) {
        const recomputed = evaluateFormula(q.answerFormula, q.seedParams);
        if (recomputed === null) {
 ...
 */
function normaliseAnswer(raw, opts) {
 ...
 */
function answersMatch(userRaw, correctRaw, tolerance = DECIMAL_TOLERANCE) {
    ...
    if (!isNaN(uFrac) && !isNaN(cNum))
        return Math.abs(uFrac - cNum) <= tolerance;
    if (!isNaN(cFrac) && !isNaN(uNum))
        return Math.abs(uNum - cFrac) <= tolerance;
    return false;
}
